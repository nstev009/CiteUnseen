...
const Analytics = () => {
  ...
  useEffect(() => {
    ...
    const loadCitationData = async () => {
      try {
        setLoading(true);
        const response = await fetch('src/assets/parking_citations.csv');
        ...
        Papa.parse(csvText, {
          ...
          complete: (results) => {
            ...
            const cleanedData = data.map(row => ({
              ...
              hour: parseInt(row['Date.Time.Hour']) || 0,
              minute: parseInt(row['Date.Time.Minute']) || 0,
              date: new Date(
                parseInt(row['Date.Year']) || 2020,
                (parseInt(row['Date.Month']) || 1) - 1,
                parseInt(row['Date.Day']) || 1
              )
            }));
  ...
  const getViolationFrequencyData = () => {
    ...
    const result = Object.entries(violationCounts)
      .map(([violation, count]) => ({ violation, count }))
      .sort((a, b) => b.count - a.count)
      .slice(0, 6); // Top 6 violations
    
    // Debug: log the data to console
    console.log('Violation data for chart:', result);
      ...
      <div className="container">
        ...
        <div className="charts-grid">
          ...
          <div className="chart-container bar-chart">
            ...
            <div style={{ width: '100%', height: '500px', paddingBottom: '60px' }}>
